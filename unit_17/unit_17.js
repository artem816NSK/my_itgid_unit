...
function t10() {
    ...
    a10.forEach(elem => {
        if(elem < 0) newA10.push(0);
        else newA10.push(elem);
    });
...
function t14() {
    ...
    if(a14.includes(a14_sym.toUpperCase()) || a14.includes(a14_sym.toLowerCase())) return true;
    return false;
}
...
function t15() {
    ...
    a15_res = a15.filter(elem => {
        if(elem.pnum != '' && elem.pnum.length == 6) return elem;
    });
